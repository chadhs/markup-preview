...
const desktop = await readFile(path.join(root, 'markup-preview.desktop'), 'utf8');
const fields = Object.fromEntries(desktop.split('\n').filter((line) => line.includes('=')).map((line) => {
  const index = line.indexOf('=');
  return [line.slice(0, index), line.slice(index + 1)];
}));
assert.equal(fields.Name, 'Markup Preview');
assert.equal(fields.Type, 'Application');
assert.equal(fields.Terminal, 'false');
assert.equal(fields.StartupWMClass, 'markup-preview');
assert.equal(fields.Icon, 'markup-preview');
...
for (const mime of ['text/org', 'text/x-org', 'text/markdown', 'text/x-markdown']) assert.ok(fields.MimeType.split(';').includes(mime));
assert.ok(fields.Keywords.includes('Emacs;'));
assert.ok(fields.Keywords.includes('Markdown;'));
...
assert.ok(sources.some((source) => source.equals(icon)), 'AppImage must contain the Markup Preview icon');
